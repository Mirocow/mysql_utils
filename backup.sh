...
backup()
{
    ...
    for DATABASE in $DATABASES; do
        ...
        mysql --defaults-file=$CONFIG_FILE --skip-column-names -B -e "$query" | awk -F"\t" '{ print $2 }' 2>> $BACKUP_DIR/$DATABASE/error.log | sed 's/AUTO_INCREMENT=[0-9]*\b//' > $BACKUP_DIR/$DATABASE/__create.sql
