...
backup()
{
    ...
    for viewName in $(mysql --defaults-file=$CONFIG_FILE $DATABASE -N -e "$query" | sed 's/|//' | awk '{print $1}'); do
        mysqldump --defaults-file=$CONFIG_FILE --skip-column-statistics $DATABASE $viewName | sed -e 's/DEFINER=[^*]*\*/\*/' >> $DATABASE_DIR/$DATABASE/__views.sql
    ...
    mysqldump --defaults-file=$CONFIG_FILE --skip-column-statistics --routines --skip-events --skip-triggers --no-create-info --no-data --no-create-db --skip-opt $DATABASE | sed -e 's/DEFINER=[^*]*\*/\*/' > $DATABASE_DIR/$DATABASE/__routines.sql
    if [ -f $DATABASE_DIR/$DATABASE/__routines.sql ]; then
        log "BACKUP:  > Exporting Routines"
    fi

    mysqldump --defaults-file=$CONFIG_FILE --skip-column-statistics --triggers --skip-events --skip-routines --no-create-info --no-data --no-create-db --skip-opt $DATABASE | sed -e 's/DEFINER=[^*]*\*/\*/' > $DATABASE_DIR/$DATABASE/__triggers.sql
    if [ -f $DATABASE_DIR/$DATABASE/__triggers.sql ]; then
        log "BACKUP:  > Exporting Triggers"
    fi

    mysqldump --defaults-file=$CONFIG_FILE --skip-column-statistics --events --skip-routines --skip-triggers --no-create-info --no-data --no-create-db --skip-opt $DATABASE | sed -e 's/DEFINER=[^*]*\*/\*/' > $DATABASE_DIR/$DATABASE/__events.sql
    ...
    for TABLE in $(eval $command); do
        ...
        if [ $(echo $data_tables_exclude_expression| grep $TABLE) ]; then
            ...
            mysqldump --defaults-file=$CONFIG_FILE --skip-column-statistics --no-data --add-drop-table --skip-events --skip-routines --skip-triggers --tab=$DATABASE_DIR/$DATABASE/ $DATABASE $TABLE
        else
            ...
            if [ ! -z "$hasGeo" ]; then
                mysqldump --defaults-file=$CONFIG_FILE --skip-column-statistics --flush-logs --default-character-set=utf8 --add-drop-table --quick --skip-events --skip-routines --skip-triggers --result-file=$DATABASE_DIR/$DATABASE/$TABLE.sql $DATABASE $TABLE
            else
                mysqldump --defaults-file=$CONFIG_FILE --skip-column-statistics --flush-logs --default-character-set=utf8 --add-drop-table --quick --skip-events --skip-routines --skip-triggers --tab=$DATABASE_DIR/$DATABASE/ $DATABASE $TABLE
            fi
        fi
